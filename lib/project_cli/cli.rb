class ProjectCli::Cli
  def call
    ...
    quit_trigger = true
    while quit_trigger
      puts "Would you like to search by region, county, difficulty? Or would you like to see all the trails?                  [region/difficulty/all/quit]"
  ...
  def search_by_difficulty
    second_level = true
    puts "Please enter difficulty: [Central Florida/Southern Florida/Northern Florida/Florida Panhandle/Miami Metro]"
    search_region = gets.strip.capitalize
    list = ProjectCli::Trail.search_by_region(search_region)
    while second_level
      puts "Trails in #{search_region} region:"
